...
@auth
    ...
    @if ($settings->push_notification_status)
        ...
        <script>
            const myDeviceKeysId = {!! json_encode(
                auth()->user()->oneSignalDevices->pluck('player_id')->all(),
            ) !!};
...
<script type="text/javascript">
    ...
    var _title =
        '@section('title')@show {{ e($settings->title) . ' - ' . __('seo.slogan') }}';
    ...
    var formats_available_verification =
        "{{ trans('general.formats_available_verification_form_w9', ['formats' => 'JPG, PNG, GIF']) }}";
    var file_size_allowed = {{ $settings->file_size_allowed * 1024 }};
    var max_size_id =
        "{{ trans('general.max_size_id') . ' ' . Helper::formatBytes($settings->file_size_allowed * 1024) }}";
    var max_size_id_lang =
        "{{ trans('general.max_size_id') . ' ' . Helper::formatBytes($settings->file_size_allowed_verify_account * 1024) }}";
    ...
    var formats_available_verification_form_w9 =
        "{{ trans('general.formats_available_verification_form_w9', ['formats' => 'PDF']) }}";
...
<style type="text/css">
    ...
    {{ $settings->button_style == 'rounded' ? '.btn, .sa-button-container button {border-radius: 50rem!important;}' : null }} @if ((auth()->check() && auth()->user()->dark_mode == 'off') || auth()->guest())
    ...
    @if ($settings->color_default != '')
        ...
        .wrapper-msg-inbox {
            scrollbar-color: #ccc @if (auth()->check() && auth()->user()->dark_mode == 'on')
                #313131
            @else
                #ebebeb
            @endif
