...
@php
  if($isSubCategory){
    $url =  url('category', [$catSlug, $subCatSlug]);
  }elseif(!$isSubCategory && isset($isCategory)){
...
<div class="btn-block mb-3">
  <span>
    ...
    <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if (request()->is('creators') || isset($isCategory) && request()->is('category/'.$catSlug.''))active-category @endif" href="{{ $url }}">
      <img src="{{url('public/img/popular.png')}}" class="mr-2" width="30" /> {{trans('general.popular')}}
    </a>

    <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if (request()->is('creators/featured') || isset($isCategory) && request()->is('category/'.$catSlug.'/featured'))active-category @endif" href="{{ $url.'/featured' }}">
      <img src="{{url('public/img/featured.png')}}" class="mr-2" width="30" /> {{trans('general.featured_creators')}}
    </a>

    <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if (request()->is('creators/more-active') || isset($isCategory) && request()->is('category/'.$catSlug.'/more-active'))active-category @endif" href="{{ $url.'/more-active' }}">
      <img src="{{url('public/img/more-active.png')}}" class="mr-2" width="30" /> {{trans('general.more_active')}}
    </a>

    <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if (request()->is('creators/new') || isset($isCategory) && request()->is('category/'.$catSlug.'/new'))active-category @endif" href="{{ $url.'/new' }}">
      <img src="{{url('public/img/creators.png')}}" class="mr-2" width="30" />  {{trans('general.new_creators')}}
    </a>

    <a class="text-muted btn btn-sm bg-white border mb-2 e-none btn-category @if (request()->is('creators/free') || isset($isCategory) && request()->is('category/'.$catSlug.'/free'))active-category @endif" href="{{ $url.'/free' }}">
