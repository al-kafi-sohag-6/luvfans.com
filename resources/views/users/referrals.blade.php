...
@php 
	...
	if (auth()->user()->referral == 1) {
		if(auth()->user()->custom_refer == 0){
			$refer_percent = $settings->percentage_referred;
		}else{
			$refer_percent = auth()->user()->custom_refer;
		}
	}else{
        ...
        <div class="col-lg-8 py-5">
          ...
          @if (auth()->user()->referral == 1)
            ...
            <p class="lead text-muted mt-0">
              {{trans('general.referrals_welcome_desc', ['percentage' => $refer_percent])}}
              <small class="d-block">
                @if ($settings->referral_transaction_limit <> 'unlimited')
                  * {{ trans_choice('general.total_transactions_per_referral', $refer_percent, ['percentage' => $refer_percent, 'total' => $settings->referral_transaction_limit]) }}
                @else
                  * {{trans('general.total_transactions_referral_unlimited', ['percentage' => $refer_percent])}}
                @endif
